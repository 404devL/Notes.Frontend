...
export class Client extends ClientBase {
     ...
     * @return Success
     */
    get(id: string, version: string): Promise<NoteDescriptionsVm> {
    ...
    protected processGet(response: Response): Promise<NoteDescriptionsVm> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as NoteDescriptionsVm;
            return result200;
            });
        } else if (status === 401) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<NoteDescriptionsVm>(null as any);
    }
     ...
     * @return No Content
     */
    delete(version: string, id: string, body: string | undefined): Promise<void> {
        let url_ = this.baseUrl + "/api/{version}/Note/{id}";
        if (version === undefined || version === null)
            throw new Error("The parameter 'version' must be defined.");
        url_ = url_.replace("{version}", encodeURIComponent("" + version));
...
export interface NoteDescriptionsVm {
    id?: string;
